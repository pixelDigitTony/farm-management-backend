...
MongoDB atomic claims, ownership tokens and expiring leases fence stale completion. A singleton worker lease keeps active compression at one across restarts. Each encode runs in a killable child process with a deadline; cancelling or stopping cannot update application records. A crash after output insertion is safe because an identical output insert reuses the unique image. The worker removes successful source files after durable completion and intermediate directories after each attempt. Cleanup sweeps terminal and abandoned files; it does not remove files for active jobs. TTL is set only after filesystem cleanup, never as a substitute for it.
...
In both API and worker shells, use the same development MongoDB, persistent `IMAGE_STAGING_DIR`, and settings from `.env.images.example`; use a private directory owned by the service account, mode 0700. Then:

```sh
npm ci
npm run build
npm run dev          # shell 1: HTTP API
npm run dev:worker   # shell 2: separate worker process
```

Production executable commands are `npm start` and `npm run start:worker`. The API and worker both use the application's `MissVBusiness` database in normal operation. The executable worker test uses only the fresh Testcontainer URI supplied by its harness.

## Same-server deployment (configuration supplied, not deployed)

The repository previously documented a single `npm start` API without a persistent volume or worker supervisor. That is insufficient for this architecture. A stateless hosting instance, an ephemeral filesystem, or two services with unrelated disks cannot run it correctly. The supplied Docker Compose configuration supports one Linux server with a durable named volume shared by two supervised processes. On a managed host, equivalent persistent shared storage and process supervision must be confirmed before enabling uploads; no provider-specific support is assumed.
...
3. Start with `docker compose -f deploy/compose.images.yaml up -d api worker`.
...
5. Verify authenticated `GET /api/images/health` reports a recent worker heartbeat. Check `docker compose -f deploy/compose.images.yaml logs worker` and a disposable uploaded test record before enabling the feature for users.

Compose provides `restart: unless-stopped`, an init process, 45-second graceful stops, a 768 MiB API memory limit, a 1536 MiB worker memory limit, and 1/2 CPU limits respectively. Worker children share the worker's cgroup, so native allocations are bounded as well as the JS heap. The persistent volume is initialized for UID 1000, private mode 0700. Back up MongoDB and the persistent staging volume together; preserve queued sources through restarts. Never use `compose down -v` for an application restart. Concurrency is intentionally restricted to 1; increase capacity through calibration rather than spawning additional encoders.
...
`tests/image-quality.test.ts` skips its native metric case only when running ordinary unit tests without `SSIMULACRA2_BIN`; the separate executable-worker integration suite fails clearly when it is missing. Unit tests cover pixel/animation/orientation/resize/alpha rules, non-monotonic candidate selection and missing-tool failure. Disposable integration covers durable admission, ready attachment, access and publishing, concurrent deduplication, expired leases, stale completion, retry exhaustion, insertion-before-completion recovery, safe cleanup and failed replacements. The actual worker tests exercise codec/metric execution and restart recovery. Frontend tests exercise pending-save blocking, stale selection, failed upload retention and existing URL/embed handling. One stale checkout query mock was repaired so existing checkout tests support both query chaining and awaiting.

For a manual browser fixture, build first, then run `FRONTEND_URL=http://127.0.0.1:5179 NODE_ENV=test EMAIL_PROVIDER=console JWT_SECRET=isolated-test-secret-not-for-production SSIMULACRA2_BIN=/absolute/path/to/ssimulacra2 npx tsx tests/support/image-server.ts`. It creates a new disposable database, seeds test credentials from `tests/support/seed.ts`, starts the worker and API on 4139, and cleans up on SIGTERM/SIGINT. Point Vite at `VITE_API_URL=http://127.0.0.1:4139/api`. This fixture must never be exposed publicly.
...
Stop new upload admission by removing the image configuration from the API environment, while keeping the image delivery code deployed. Stop the worker gracefully or allow existing queued work to drain first. Revert frontend upload controls if necessary, but retain support for existing `/api/images/<id>` references. A complete rollback to code predating image delivery will break those references: retain a compatibility delivery service or first migrate affected references through an explicitly authorized migration. Do not delete collections, documents, drafts, snapshots or the persistent volume. Existing URL/embed-only records need no migration.
