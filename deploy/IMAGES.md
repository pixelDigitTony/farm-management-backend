...
Every shared image uploader offers **Upload new** and **Media library**. New files can be cropped, rotated, flipped, resized without enlargement, and adjusted for brightness, contrast, and saturation in the browser. Upload starts only after the final preview is confirmed. Unedited files keep their original bytes; edited files are submitted as PNG and still pass through the existing backend validation and optimization pipeline. The preserve-detail option skips automatic backend resizing, while retaining explicit user edits.

`GET /api/images/library?scope=catalog|menu|landing-page&limit=24&before=<imageId>` lists completed stored images for the authenticated business, newest first. It requires the chosen feature's view permission, limits pages to 48 entries, and returns metadata/URLs without binary payloads. Existing images appear automatically, including uploads from other company users and features; no migration or upload-job retention is required. Existing private delivery and record-attachment checks remain in force. Uploads can be saved to the library only, and editing a library image creates a new upload rather than changing existing references. Library selection alone does not publish an image.
