...
Generic resource create/update routes now validate owned source contacts, pig batches, measurement targets, menu recipes and every recipe ingredient. Tests cover foreign/missing/malformed IDs, rejected writes leaving existing records intact, repeated owned ingredient IDs, omitted fields, empty ingredient lists and nullable optional references. This is tenant validation at write time; it does not provide a foreign-key guarantee against concurrent deletion or repair existing invalid records.

Remaining work includes source-linked expense protection, a reference audit of posting operations, concurrent deletion integrity, operation idempotency, stock-lot consistency, legacy stuck-order recovery, typed repositories/models, contract/security fuzzing, fault recovery, migrations and meaningful mutation/coverage targets. Current suites do not establish all of these properties.
